namespace RSDK.API
{
    ...
    public static class Auth
    {
        ...
        public static bool32 GetUsername(String* username) => APITable.GetUsername(username);
    }
#endif
    ...
    public static class Storage
    {
        ...
        public static bool32 LoadFile(char8* fileName, void* buffer, uint32 size) => RSDKTable.LoadUserFile(fileName, buffer, size);
        public static bool32 SaveFile(char8* fileName, void* buffer, uint32 size) => RSDKTable.SaveUserFile(fileName, buffer, size);
        ...
        public static bool32 GetNoSave() => APITable.GetNoSave();
        ...
        public struct UserDB
        {
            ...
            public bool32 Matches(Storage.UserDB other) => id == other.id;
            public bool32 Matches(Storage.UserDB* other) => other != null ? id == other.id : id == (.)(-1);
            ...
            public int32 SortedRowCount() => APITable.GetSortedUserDBRowCount(id);
            public int32 GetSortedRowID(uint16 row) => APITable.GetSortedUserDBRowID(id, row);

            // Rows
            public int32 AddRow() => APITable.AddUserDBRow(id);
            ...
            public uint32 GetRowUUID(uint16 row) => APITable.GetUserDBRowUUID(id, row);
            public int32 GetRowByID(uint32 uuid) => APITable.GetUserDBRowByID(id, uuid);
