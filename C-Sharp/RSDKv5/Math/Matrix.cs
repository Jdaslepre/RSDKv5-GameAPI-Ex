...
public unsafe struct Matrix
{
    ...
    public void SetIdentity() { fixed (Matrix* m = &this) { RSDKTable->SetIdentityMatrix(m); } }
    public void TranslateXYZ(int x, int y, int z, uint setIdentity = 1) { fixed (Matrix* m = &this) RSDKTable->MatrixTranslateXYZ(m, x, y, z, setIdentity); }
    public void ScaleXYZ(int x, int y, int z) { fixed (Matrix* m = &this) RSDKTable->MatrixScaleXYZ(m, x, y, z); }
    public void RotateX(int angle) { fixed (Matrix* m = &this) RSDKTable->MatrixRotateX(m, (short)angle); }
    public void RotateY(int angle) { fixed (Matrix* m = &this) RSDKTable->MatrixRotateX(m, (short)angle); }
    public void RotateZ(int angle) { fixed (Matrix* m = &this) RSDKTable->MatrixRotateX(m, (short)angle); }
    public void RotateXYZ(int x, int y, int z) { fixed (RSDK.Matrix* m = &this) { RSDKTable->MatrixRotateXYZ(m, (short)x, (short)y, (short)z); } }
    public void Inverse() { fixed (Matrix* m = &this) RSDKTable->MatrixInverse(m, m); }
